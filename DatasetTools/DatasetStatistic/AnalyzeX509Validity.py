...
class ExtractFeatures(object):
    ...
    def __init__(self):
        ...
        self.valid_cert = 0
        self.not_valid_cert = 0
        self.unknown_valid_cert = 0
    """
    --------------------- X509 logs. ------------------------
    """
    def x509_logs(self, dataset_path_to_logs):
        ...
        print "     << x509 statistic:"
        print "     << valid cert:", self.valid_cert
        print "     << not valid cert:", self.not_valid_cert
        print "     << unknow cert:", self.unknown_valid_cert
        ...
        try:
            with open(dataset_path_to_logs + x509_log) as f:
                ...
                for line in f:
                    ...
                    new_x509_split = new_line.split('	')
                    # Certificate time validation
                    try:
                        if float(new_x509_split[0]) > float(new_x509_split[6]) and float(new_x509_split[0]) < float(new_x509_split[7]):
                            self.valid_cert += 1
                        else:
                            # print str(float(new_x509_split[6])) +' - ' + str(float(new_x509_split[0])) + '- '+ str(float(new_x509_split[7]))
                            self.not_valid_cert += 1
                    except:
                        self.unknown_valid_cert += 1
